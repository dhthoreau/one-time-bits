...
#define MIN_PAD_SYNCHRONIZATION_INTERVAL		1
#define MAX_PAD_SYNCHRONIZATION_INTERVAL		G_MAXINT64
...
static void otb_bitkeeper_init(OtbBitkeeper *bitkeeper)
{
	...
	bitkeeper->priv->pad_synchronization_interval=0;
...
static void otb_bitkeeper_synchronize_pads_of_friend_from_unique_id(OtbUniqueId *friend_unique_id, OtbLoopableThread *loopable_thread)
{
	if(loopable_thread->continue_looping)
	{
		...
		if(friend!=NULL)
		{
			...
			// FARE - La synchronizazione dei blocchi va qui.
