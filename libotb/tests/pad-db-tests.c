...
static void test_encryption_fails_due_to_not_enough_pad_bytes()
{
	...
	g_assert(!otb_encrypt(cipher_context, EXPECTED_MESSAGE, MESSAGE_SIZE, &encrypted_bytes, &encrypted_bytes_size));
...
static void test_encryption_with_one_pad()
{
	...
	g_assert_cmpint(0, ==, memcmp(otb_unique_id_get_bytes(expected_unique_id), (OtbUniqueId*)(encrypted_bytes+sizeof(unsigned char)), OTB_UNIQUE_ID_BYTES_SIZE));
...
static void test_decryption_fails_due_to_unsupported_file_format()
{
	...
	OtbCipherContext *cipher_context=otb_cipher_context_new(pad_db);
	g_assert(!otb_decrypt(cipher_context, input_bytes, sizeof FORMAT_VERSION+OTB_UNIQUE_ID_BYTES_SIZE, &decrypted_bytes, &decrypted_bytes_size));
...
static size_t otb_encrypt_file_for_two_pad_test(OtbPadDb *pad_db, size_t chunk_size, const void *message, size_t message_size, unsigned char **encrypted_message_out)
{
	...
	for(const unsigned char *current_message_byte=(unsigned char*)message; current_message_byte<message_end_byte; current_message_byte+=chunk_size)
	{
		...
		g_assert(otb_encrypt(cipher_context, current_message_byte, chunk_size_to_encrypt, &encrypted_buffer, &encrypted_buffer_size));
...
static void test_encryption_decryption_with_two_pads_varying_chunk_size()
{
	...
	for(size_t chunk_size=1; chunk_size<=MESSAGE_SIZE+33; chunk_size+=15)
...
void otb_add_pad_db_tests()
{
	...
	otb_add_test_func("/pad-db/test_encryption_decryption_with_two_pads_varying_chunk_size", test_encryption_decryption_with_two_pads_varying_chunk_size);
}
