...
static void otb_sym_cipher_class_init(OtbSymCipherClass *klass)
{
	...
	g_object_class_install_property(object_class, PROP_CIPHER, g_param_spec_string(OTB_SYM_CIPHER_PROP_CIPHER, _("Cipher"), _("Name of the cipher to use"), OTB_SYM_CIPHER_DEFAULT_CIPHER, G_PARAM_READWRITE | G_PARAM_CONSTRUCT/*FARE _ONLY*/));
	g_object_class_install_property(object_class, PROP_MESSAGE_DIGEST, g_param_spec_string(OTB_SYM_CIPHER_PROP_MESSAGE_DIGEST, _("Message digest"), _("Message digest to use for hashes"), OTB_SYM_CIPHER_DEFAULT_MESSAGE_DIGEST, G_PARAM_READWRITE | G_PARAM_CONSTRUCT/*FARE _ONLY*/));
	g_object_class_install_property(object_class, PROP_HASH_ITERATIONS, g_param_spec_uint(OTB_SYM_CIPHER_PROP_HASH_ITERATIONS, _("Hash iterations"), _("Number of iterations to go through when hashing"), 1, G_MAXUINT, OTB_SYM_CIPHER_DEFAULT_HASH_ITERATIONS, G_PARAM_READWRITE | G_PARAM_CONSTRUCT/*FARE _ONLY*/));
...
static void otb_sym_cipher_set_key(OtbSymCipher *sym_cipher, const unsigned char *key, size_t key_size)
{
	if(sym_cipher->priv->key!=NULL)
		otb_free_locked(sym_cipher->priv->key);
...
GBytes *otb_sym_cipher_hash_passphrase(const OtbSymCipher *sym_cipher, const char *passphrase, OtbSymCipherSalt **sym_cipher_salt_out)
{
	...
	*sym_cipher_salt_out=g_slice_new(OtbSymCipherSalt);
	unsigned char *hash_bytes=g_new(unsigned char, hash_size);
	if(G_LIKELY(otb_random_bytes((*sym_cipher_salt_out)->salt, sizeof (*sym_cipher_salt_out)->salt) && PKCS5_PBKDF2_HMAC(passphrase, strlen(passphrase), (*sym_cipher_salt_out)->salt, sizeof (*sym_cipher_salt_out)->salt, sym_cipher->priv->hash_iterations, sym_cipher->priv->message_digest_impl, hash_size, hash_bytes)))
...
gboolean otb_sym_cipher_validate_passphrase(const OtbSymCipher *sym_cipher, const char *passphrase, GBytes *passphrase_hash, const OtbSymCipherSalt *sym_cipher_salt)
{
	...
	unsigned char *hash_bytes=g_new(unsigned char, hash_size);
	otb_sym_cipher_lock_read(sym_cipher);
	if(PKCS5_PBKDF2_HMAC(passphrase, strlen(passphrase), sym_cipher_salt->salt, sizeof sym_cipher_salt->salt, sym_cipher->priv->hash_iterations, sym_cipher->priv->message_digest_impl, hash_size, hash_bytes) && otb_smemcmp(passphrase_hash_bytes, hash_bytes, hash_size)==0)
		ret_val=TRUE;
	otb_sym_cipher_unlock_read(sym_cipher);
	g_free(hash_bytes);
	return ret_val;
...
{
	OtbSymCipherContext *sym_cipher_context=g_new(OtbSymCipherContext, 1);
	EVP_CIPHER_CTX_init(sym_cipher_context);
	if(G_UNLIKELY(EVP_EncryptInit_ex(sym_cipher_context, sym_cipher_impl, NULL, key, iv)<=0))
	{
...
static OtbSymCipherContext *otb_sym_cipher_init_decryption_openssl(const EVP_CIPHER *sym_cipher, const unsigned char *key, const unsigned char *iv)
{
	OtbSymCipherContext *sym_cipher_context=g_new(OtbSymCipherContext, 1);
	EVP_CIPHER_CTX_init(sym_cipher_context);
	if(G_UNLIKELY(EVP_DecryptInit_ex(sym_cipher_context, sym_cipher, NULL, key, iv)<=0))
	{
		otb_sym_cipher_context_free(sym_cipher_context);
		char *error=otb_openssl_errors_as_string();
...
gboolean otb_sym_cipher_unwrap_key(OtbSymCipher *sym_cipher, GBytes *wrapped_key, const char *passphrase, const OtbSymCipherSalt *sym_cipher_salt)
{
	...
	if(PKCS5_PBKDF2_HMAC(passphrase, strlen(passphrase), sym_cipher_salt->salt, sizeof sym_cipher_salt->salt, sym_cipher->priv->hash_iterations, sym_cipher->priv->message_digest_impl, key_and_iv_size(sym_cipher), wrapping_key_and_iv))
	{
		...
		otb_sym_cipher_dispose_decryption_buffer(key_bytes);
	}
	otb_sym_cipher_unlock_write(sym_cipher);
...
GBytes *otb_sym_cipher_wrap_key(const OtbSymCipher *sym_cipher, const char *passphrase, OtbSymCipherSalt **sym_cipher_salt_out)
{
	...
	*sym_cipher_salt_out=g_slice_new(OtbSymCipherSalt);
	if(G_UNLIKELY(otb_random_bytes((*sym_cipher_salt_out)->salt, sizeof (*sym_cipher_salt_out)->salt)))
	{
		otb_sym_cipher_lock_read(sym_cipher);
		unsigned char *wrapping_key_and_iv=g_new(unsigned char, key_and_iv_size(sym_cipher));
		if(PKCS5_PBKDF2_HMAC(passphrase, strlen(passphrase), (*sym_cipher_salt_out)->salt, sizeof (*sym_cipher_salt_out)->salt, sym_cipher->priv->hash_iterations, sym_cipher->priv->message_digest_impl, key_and_iv_size(sym_cipher), wrapping_key_and_iv))
		{
			unsigned char *wrapped_key_bytes=otb_sym_cipher_create_encryption_buffer(sym_cipher, sym_cipher->priv->key_size);
...
size_t otb_sym_cipher_encrypt_next(OtbSymCipherContext *sym_cipher_context, const void *plain_bytes, size_t plain_bytes_size, unsigned char *encrypted_bytes)
{
	...
	if(G_UNLIKELY(EVP_EncryptUpdate(sym_cipher_context, encrypted_bytes, &encrypted_bytes_size, plain_bytes, plain_bytes_size)<=0))
	{
		char *error=otb_openssl_errors_as_string();
...
size_t otb_sym_cipher_decrypt_next(OtbSymCipherContext *sym_cipher_context, const unsigned char *encrypted_bytes, size_t encrypted_bytes_size, void *plain_bytes)
{
	...
	if(G_UNLIKELY(EVP_DecryptUpdate(sym_cipher_context, plain_bytes, &plain_bytes_size, encrypted_bytes, encrypted_bytes_size)<=0))
	{
		char *error=otb_openssl_errors_as_string();
...
{
	int encrypted_bytes_size;
	if(G_UNLIKELY(EVP_EncryptFinal_ex(sym_cipher_context, encrypted_bytes, &encrypted_bytes_size)<=0))
	{
		char *error=otb_openssl_errors_as_string();
...
{
	int plain_bytes_size;
	if(G_UNLIKELY(EVP_DecryptFinal_ex(sym_cipher_context, plain_bytes, &plain_bytes_size)<=0))
	{
		char *error=otb_openssl_errors_as_string();
...
{
	*encrypted_bytes_out=otb_sym_cipher_create_encryption_buffer(sym_cipher, plain_bytes_size);
...
{
	*plain_bytes_out=otb_sym_cipher_create_decryption_buffer(sym_cipher, encrypted_bytes_size);
